...
const npmDir = path.join(dirname, "node_modules", "chartiq");
const chartiqDir = fs.existsSync(npmDir) ? npmDir : path.join(dirname, "../");
...
export default {
	...
	plugins: [
		...
		new CopyPlugin({
			patterns: [
				{
					from: path.join(chartiqDir, "js/thirdparty/html2canvas.min.js"),
		...
		new HtmlWebpackPartialsPlugin({
			path: path.join(dirname, "src", "chart-context-markup.html"),
			location: "cq-context"
		}),
		/**
