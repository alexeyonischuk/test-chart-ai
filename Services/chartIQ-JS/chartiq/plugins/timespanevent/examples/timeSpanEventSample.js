...
let alertPng, newsPng;

const imageReady = new Promise((resolve) => {
	if (import.meta.webpack) {
		Promise.all([
			import(/* webpackMode: "eager" */ "../images/alert.png"),
			import(/* webpackMode: "eager" */ "../images/news.png")
		]).then(resolve);
	} else if (typeof define === "function" && define.amd) {
		define(["../images/alert.png", "../images/news.png"], resolve);
	} else {
		resolve([
			new URL("../images/alert.png", import.meta.url).href,
			new URL("../images/news.png", import.meta.url).href
		]);
	}
})
	.then((modules) => modules.map((m) => m.default || m))
	.then(([alert, news]) => {
		alertPng = alert;
		newsPng = news;
	});
...
CIQ.TimeSpanEventSample.prototype.showTimeSpanEvent = function (eventName) {
	...
	for (var i = eventData.length - 1; i >= 0; i--) {
		...
		if (display.randomLength)
			length = Math.round(15 + CIQ.random() * (display.length / 2));
		...
		if (datum.subChildren) {
			...
			for (var index = startPoint.tick; index < endPoint.tick; index += 3) {
				...
				if (!quote) return;
				var p = CIQ.round(
					quote.Low + Math.abs(CIQ.random() * (quote.High - quote.Low)),
					chart.decimalPlaces
				);
