...
const cssReady = new Promise((resolve) => {
	if (import.meta.webpack) {
		// webpack 5
		import(/* webpackMode: "eager" */ "./timespanevent.css").then(resolve);
	} else if (typeof define === "function" && define.amd) {
		define(["./timespanevent.css"], resolve);
	} else if (typeof window !== "undefined") {
		// no webpack
		CIQ.loadStylesheet(
			new URL("./timespanevent.css", import.meta.url).href,
			resolve
		);
	} else resolve();
}).then((m) => CIQ.addInternalStylesheet(m, "timespanevent"));
 ...
 * @param {object} [params.customConstants] Modifies the values of the constants in
...
CIQ.TimeSpanEventPanel = function (params) {
	 ...
	 * @type boolean
	 * @alias alwaysZoom
	 * @memberof CIQ.TimeSpanEventPanel
	 * @default true
	 * @since 8.0.0
	 */
	this.alwaysZoom =
	...
	this.display = function (on) {
		...
		if (!panel) {
			...
			panel.icons.classList.add("lce-panel");
		} else {
	 ...
	 * 		"singleEvent".
	 * @param {string} [spanEvent.infoPanel] Specifies the panel where information pop-up
...
if (CIQ.UI && CIQ.UI.Layout) {
	...
	CIQ.UI.TimeSpanEvent = function (context, params) {
		const self =
			this instanceof CIQ.UI.TimeSpanEvent
				? this
				: Object.create(CIQ.UI.TimeSpanEvent.prototype);
		...
		function init() {
			...
			context.stx.addEventListener("symbolChange", () => {
				context.stx.timeSpanEventPanel.eventData.slice(0).forEach((i) => {
