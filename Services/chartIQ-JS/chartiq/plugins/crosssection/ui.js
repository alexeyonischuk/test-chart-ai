 ...
 * @param {boolean} [params.enableTimeSeries=false] Specifies whether an option to link to a time series
 * 		chart should appear when right-clicking on a point on the chart.
 * @param {boolean} [params.enableToggleLabel=false] Specifies whether an option to show a label above the point
 * 		on the chart when right-clicking on a point on the chart.
 ...
 * - 8.5.0 Added the `enableToggleLabel` parameter. `cq-time-series` attribute decommissioned.
 *
 * @example
...
function CurveEdit(node, context, params = {}) {
	const self =
		this instanceof CurveEdit ? this : Object.create(CurveEdit.prototype);
	...
	self.curveContext = self.node.ownerDocument.querySelector("cq-curve-context");
	if (self.curveContext) self.curveContext.classList.add("ciq-context-menu");
	self.pathToTimeSeries =
	...
	if (params.enableTimeSeries) {
		self.curveContext
			.querySelector('[stxtap="CurveEdit.launchTimeSeries()"]')
			.classList.add("active");
		stx.addEventListener("curveEdit", (params) => self.showContext(params));
		if (params.timeSeriesSymbologyEncoder)
			self.timeSeriesSymbologyEncoder = params.timeSeriesSymbologyEncoder;
		else if (CIQ.Market && CIQ.Market.Symbology)
			self.timeSeriesSymbologyEncoder =
				CIQ.Market.Symbology.encodeTermStructureInstrumentSymbol;
		else self.timeSeriesSymbologyEncoder = (s, i) => s + " " + i;
	}
	if (params.enableToggleLabel) {
		self.curveContext
			.querySelector('[stxtap="CurveEdit.toggleLabel()"]')
			.classList.add("active");
		if (!params.enableTimeSeries)
			stx.addEventListener("curveEdit", (params) => self.showContext(params));
	}
	context.advertiseAs(self, "CurveEdit");
 ...
 *
 * @memberof CIQ.UI.CurveEdit
 * @alias CIQ.UI.CurveEdit.toggleLabel
 * @since 8.5.0
 */
CurveEdit.prototype.toggleLabel = function () {
	const { highlighted, labeledPoints } = this.context.stx.crossSection;
	const { curve, instrument, subcurve } = highlighted;

	this.curveContext.close();

	for (let i = labeledPoints.length - 1; i >= 0; i--) {
		const point = labeledPoints[i];
		if (
			point.curve === curve &&
			point.subcurve === subcurve &&
			point.instrument === instrument
		) {
			labeledPoints.splice(i, 1);
			return;
		}
	}
	labeledPoints.push(highlighted);
};
 ...
 * @param {HTMLElement} params.node The UI element that has triggered the freshness edit dialog.
 *
...
CIQ.UI.Layout.prototype.showFreshnessEdit = function ({ node }) {
	let dialog = node.ownerDocument.querySelector("cq-freshness-dialog");
 ...
 * @since 8.3.0
 */
CIQ.UI.Layout.prototype.setFilter = function (obj, name, value, type) {
	let filter = this.context.stx.layout.filter || {};
	...
	if (filter[name] instanceof Array) {
		...
		filter[name] = filter[name].slice(); // trigger observer
	} else filter[name] = value;

	this.context.stx.crossSection.setFilters(filter);
};
...
CIQ.UI.Layout.prototype.showHistoricalComparisonDialog = function ({ e }) {
	const dialog = e.target.ownerDocument.querySelector(
		"cq-historical-comparison-dialog"
	);
...
CIQ.UI.Layout.prototype.showEntityComparison = function ({ e }) {
	...
	const menu = doc.querySelector(".ciq-entity-comparison cq-menu");
	const detachedDropdown = doc.querySelector(
 ...
 * - 8.3.0 Deprecated.
 */
CIQ.UI.Layout.prototype.setDataField = function (obj, field) {
	this.setYaxisField(obj, field);
};
 ...
 * 	</div>
 */
CIQ.UI.Layout.prototype.setYaxisField = function (obj, field, aggOperator) {
 ...
 * @since 8.3.0
 */
CIQ.UI.Layout.prototype.setXaxisField = function (obj, field) {
 ...
 * @since 8.3.0
 */
CIQ.UI.Layout.prototype.setGroupField = function (obj, field) {
	const { crossSection, layout } = this.context.stx;
	let trailLength;
	if (field === "_originalKey") {
		if (!layout.trailLength && crossSection._prevTrailLength)
			trailLength = crossSection._prevTrailLength || 1;
	} else {
		if (layout.trailLength) crossSection._prevTrailLength = layout.trailLength;
		trailLength = 0;
	}
	if (trailLength !== layout.trailLength)
		crossSection.setTrailLength(trailLength);
	crossSection.setGroupField(field);
};
...
CIQ.UI.Layout.prototype.getShading = function (node) {
	...
	this.addObserver({
		...
		base: stx,
		path: `layout.${prop}`,
		listener: observerCallback(node, className)
	});
};
 ...
 * @since 7.4.0
 */
CIQ.UI.Layout.prototype.setShading = function (obj, field) {
	const { crossSection, layout } = this.context.stx;
...
CIQ.UI.Layout.prototype.getXAxisScaling = function (node) {
	...
	const listener = (obj) => {
		this.setCheckedState(node, obj.value === "scaled");
	};
	const prop = "spacingType";
	if (typeof layout[prop] !== "string")
		layout[prop] = crossSection.initialSettings[prop];
	this.addObserver({ node, base: stx, path: `layout.${prop}`, listener });
};
 ...
 * @since 7.4.0
 */
CIQ.UI.Layout.prototype.setXAxisScaling = function (obj, field) {
...
/**
 * Gets the value of the property that determines whether cross section display labels are shown.
 *
 * @param {HTMLElement} node The recent updates control, which turns label display
 * 		on or off.
 *
 * @memberof CIQ.UI.Layout
 * @private
 * @since 8.5.0
 */
CIQ.UI.Layout.prototype.getPointLabels = function (node) {
	const { stx } = this.context;
	const { crossSection, layout } = stx;
	const className = this.params.activeClassName;
	const prop = "pointLabels";
	if (typeof layout[prop] !== "boolean") layout[prop] = false;
	this.addObserver({
		node,
		base: stx,
		path: `layout.${prop}`,
...
/**
 * Sets the value of the cross section display labels property.
 *
 * @param {CIQ.UI~Setter|null} obj Either null or an object containing the user interface element,
 *		trigger event, and additional param object.
 * @param {String} field
 ...
 * @since 8.5.0
 */
CIQ.UI.Layout.prototype.setPointLabels = function (obj, field) {
	const { layout, crossSection } = this.context.stx;
	crossSection.setPointLabels(!layout.pointLabels);
};
 ...
 * @since 7.4.0
 */
CIQ.UI.Layout.prototype.setFreshPoints = function (obj, field) {
	const { layout, crossSection } = this.context.stx;
 ...
 * @since 7.4.0
 */
CIQ.UI.Layout.prototype.setUpdateStamp = function (obj, field) {
	const { layout, crossSection } = this.context.stx;
 ...
 * @since 7.4.0
 */
CIQ.UI.Layout.prototype.setUpdateAnimations = function (obj, field) {
	const { layout, crossSection } = this.context.stx;
...
CIQ.UI.Layout.prototype.getTimelineDateSelector = function (node) {
	this.addObserver({
		...
		path: "layout.timelineDateSelector",
		listener: observerCallback(node, this.params.activeClassName)
	});
};
 ...
 * @since 8.3.0
 */
CIQ.UI.Layout.prototype.setTimelineDateSelector = function (obj, field) {
	const { stx } = this.context;
	const { layout } = stx;
...
CIQ.CrossSection.HUD = function (...args) {
	return new HUD(...args);
};

class CrosssectionMenu extends CIQ.UI.ContextTag {
	setContext(uiContext) {
		const { config, stx } = uiContext;

		this.buildUI(config, stx);

		// let ui component initialize prior setting up events
		setTimeout(() => {
			this.applyStyleAdjustments();
			this.setupEvents(stx);
		});
	}

	buildUI(config) {
		this.className = "ciq-dropdowns";
		this.setAttribute("cq-analytics-fields", "");

		let result = "";

		const menuMarkup = (label) => `
			<span class="ciq-menu-label">${label[0]}:</span>
		...
		if (config.menuXaxisField && config.menuXaxisField.length)
			result += menuMarkup("Xaxis");
		if (config.menuYaxisField && config.menuYaxisField.length)
			result += menuMarkup("Yaxis");
		if (config.menuGroupField && config.menuGroupField.length)
			result += menuMarkup("Group");

		if (config.menuFilterField && config.menuFilterField.length) {
			result += menuMarkup("Filter")
		...
		if (config.showTrailsMenu)
			result += `
			<span class="ciq-menu-label">Trails	:</span>
			<cq-menu class="ciq-menu ciq-trails">
			<cq-clickable class="ciq-menu-field"></cq-clickable>
			<cq-menu-dropdown class="ciq-value-dropdown cq-no-close">
				<cq-item>
					<input
						type="range"
						min="${trailLengthMin}"
						max="${trailLengthMax}"
						class="ciq-trail-length"
					>
				</cq-item>
			</cq-menu-dropdown>
		</cq-menu>`;
		...
		if (symbolInputType) {
			const entityComparison = this.closest("cq-context").querySelector(
				'[stxtap="Layout.showEntityComparison()"]'
			);
			entityComparison.innerHTML = "Add " + symbolInputType;
		}
	}
	...
	setupEvents(stx) {
		...
		this.sliderArray = Array.from(
			contextEl.querySelectorAll("cq-double-slider")
		);

		stx.append("draw", () => {
			this.sliderArray.forEach((el) => {
				const bounds = stx.crossSection.findExtrema(el.name);
				bounds.min = Math.floor(bounds.min);
				bounds.max = Math.ceil(bounds.max);
				el.setBounds(bounds);
			});
		});
		...
		this.sliderArray.forEach((el) => {
			stx.addEventListener("symbolChange", function () {
				el.setValue(this.layout.filter[el.name]);
			});
		...
		const trailLengthInput =
			contextContainer.querySelector(".ciq-trail-length");
		const trailLengthLabel = contextContainer.querySelector(
			".ciq-trails .ciq-menu-field"
		);

		if (trailLengthInput) {
			trailLengthInput.style.width = "170px";
			trailLengthInput.addEventListener("input", () => {
				const { value } = trailLengthInput;
				if (value) stx.layout.groupField = "_originalKey";
				stx.crossSection.setTrailLength(+value);
			});

			trailLengthLabel.innerText = stx.layout.trailLength;
			CIQ.UI.observeProperty("trailLength", stx.layout, (obj) => {
				trailLengthInput.value = obj.value;
				trailLengthLabel.innerText = obj.value;
			});
		}
	}
	...
	applyStyleAdjustments() {
		...
		Array.from(contextEl.querySelectorAll("cq-double-slider")).forEach((el) => {
			el.closest("cq-menu-dropdown").style.width = "400px";
			el.querySelector(".ciq-double-slider-text").style.left = "75px";
			el.querySelector(".ciq-double-slider-range").style.width = "44%";
		});
	}
}
CIQ.UI.addComponentDefinition("cq-crossection-menu", CrosssectionMenu);

export function setConfigBasedUI(context) {
	const templateMenu = context.querySelector("cq-context .ciq-dropdowns");
	const menu = document.createElement("cq-crossection-menu");
	templateMenu.parentNode.replaceChild(menu, templateMenu);
}
