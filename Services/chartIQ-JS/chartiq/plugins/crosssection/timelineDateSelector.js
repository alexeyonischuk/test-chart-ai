...
const imageReady = new Promise((resolve) => {
	if (import.meta.webpack) {
		import(/* webpackMode: "eager" */ "./images/lock.svg").then(resolve);
	} else if (typeof define === "function" && define.amd) {
		define(["./images/lock.svg"], resolve);
	} else resolve(new URL("./images/lock.svg", import.meta.url).href);
}).then((m) => m.default || m);
...
class CurveControls extends CIQ.UI.BaseComponent {
	initialize({ stx }) {
		...
		this.addDialog = stx.container.ownerDocument.querySelector(
 ...
 * 		Requires the [cq-message-toaster]{@link WebComponents.MessageToaster} web component.
...
class TimelineDateSelector {
	constructor({
		...
		if (glyphs.lockImg) this.lockIcon.src = glyphs.lockImg;
		else imageReady.then((img) => (this.lockIcon.src = img));
		this.stx = stx;
		...
		selector.chart.yaxisMarginMultiplier = 1;
	...
	copyData() {
		...
		setTimeout(() => {
			...
			selector.chart.dataSet = chart.dataSet.map((quote) => {
				...
				if (spread === null || isNaN(spread)) this.anyNullInDataSet = true;
	...
	drawSelector() {
		...
		Object.entries(curves).forEach(
			([curve, { Date: curveDate, color, _computedColor, relativeDate }]) => {
				...
				if (relativeDate && this.lockIcon.src) {
					context.drawImage(
						this.lockIcon,
						pixel + lockBuffer,
...
CIQ.CrossSection.TimelineDateSelector = function (...args) {
	return new TimelineDateSelector(...args);
};
