...
const cssReady = new Promise((resolve) => {
	if (import.meta.webpack) {
		// webpack 5
		import(/* webpackMode: "eager" */ "./crosssection.css").then(resolve);
	} else if (typeof define === "function" && define.amd) {
		define(["./crosssection.css"], resolve);
	} else if (typeof window !== "undefined") {
		// no webpack
		CIQ.loadStylesheet(
			new URL("./crosssection.css", import.meta.url).href,
			resolve
		);
	} else resolve();
}).then((m) => CIQ.addInternalStylesheet(m, "crosssection")); // a framework, such as Angular, may require style addition
 ...
 * 		sort.
 * @param {function} [params.pointAttributes] Custom function to add values to additional dimensions,
 * 		such as color, size, shape, and text.  Function should return an object with keys of any or all
 * 		of `color`, `size`, `shape`, or `text`.  Values for these keys should be valid; e.g. a recognized
 * 		css color, a multiplier of the default size (4x candlewidth), the terms	`circle`/`square`/`diamond`,
 * 		or the text to be displayed when the point is selected, respectively.
 ...
 *
 * @param {string} [params.chartType="crosssection"] Other option "crosssection_mountain"
 * @param {number} [params.trailLength] Number of historic data points that should display trails.
 * @param {string} [params.trailColor] Trail color override. By default, trails are rendered with the same color as
 * 		the leading data point.
 * @param {number} [params.trailTension=0.8] Spline tension (i.e., the smoothness of the fitted curve) for historic trail curves. The higher the tension, the more closely the curve approximates the set of line segments that connect the data points.
 * @param {object} [center] An object defining the center of the chart.
 * @param {number} [params.center.x] Value for the x-coordinate of the centerpoint of chart.
 * @param {number} [params.center.y] Value for the y-coordinate of the centerpoint of chart.
 * @param {string} [params.center.color] Color of lines that draw the center crosshair.
 * 		Center is drawn only if color, center.x, and center.y are available.
 * @param {object} [params.center.lineParams] Parameter object for center crosshair lines. See the last parameter
 * 		of the [Chart Engine plotLine method]{@link CIQ.ChartEngine.plotline}.
 * @param {boolean} [params.avoidDuplicates] Specifies whether to avoid creating duplicate historic data points due to missing
 * 		trading days.
 * @param {boolean} [params.scalePoints=true] Specifies whether to scale vertex points size with zoom.
 * @param {string} [params.xAxisType] Set to `regular` to generate evenly-spaced x-axis ticks; by default, will
 * 		generate ticks corresponding to instrument values. If there is a custom sorting on the instruments (params.sortFunction),
 ...
 * - 8.7.0 Added `trailLength`, `trailColor`, `trailTension`,
 * 		`center.x`, `center.y`, `center.color`, `center.lineParams`, `avoidDuplicates`, and `scalePoints` parameters.
...
CIQ.CrossSection = function (params) {
	...
	this.initialSettings = {
		...
		groupField:
			params.groupField || (params.trailLength && "_originalKey") || "",
	...
	this.avoidDuplicates = params.avoidDuplicates;
	this.trailTension = params.trailTension;
	this.trailColor = params.trailColor;
	this.center = params.center || {};
	this.scalePoints = params.scalePoints;
	this.xAxisType = params.xAxisType;
	this.currentDate = new Date(); // for checking for date rollover
	this.crosshairBreakDistance = 0; // Used for keyboard control of crosshair. The distance needed to break free when snapped to a plot point.
	...
	stx.container.classList.add("stx-crosshair-cursor-on"); // turn on crosshair by default
	...
	stx.addEventListener("symbolChange", function ({ symbol, action }) {
		...
		if (curves._main_curve && action === "master") {
			...
			if (this.layout.trailLength) {
				crossSection.setTrailLength();
			}
		}
...
CIQ.CrossSection.prototype.zoomSet = function (newCandleWidth, chart) {
	if (isNaN(newCandleWidth)) return false;
	...
	if (!instruments || instruments.length < 2) return true;

	const leftXBound = this.pixelFromInstrument(instruments[0]);
	const rightXBound = this.pixelFromInstrument(
...
CIQ.CrossSection.prototype.calculateInstrumentSpacing = function (
	...
	if (type === "uniform") {
		const isHistogram = this.stx.mainSeriesRenderer.params.histogram;
		const len = isHistogram ? instruments.length : instruments.length - 1;
		let candleWidth = width / len;
		if (isHistogram) {
			spacing._buffer += candleWidth / 2;
		}
		for (let i = 0; i < instruments.length; i++) {
			let instrument = instruments[i];
 ...
 *
 * @param {boolean} bool True if displaying labels for all points.
 *
 * @memberof CIQ.CrossSection
 * @since  8.5.0
 */
CIQ.CrossSection.prototype.setPointLabels = function (bool) {
	this.labeledPoints = [];
	this.stx.layout.pointLabels = bool;
	this.stx.changeOccurred("layout");
	this.stx.draw();
};
 ...
 *
 * @param {number} [length] Length of historic trails. Defaults to plugin set trailLength
 *
 * @memberof CIQ.CrossSection
 * @since 8.7.0
 */
CIQ.CrossSection.prototype.setTrailLength = function (length) {
	const { stx, curves } = this;

	if (!length && !stx.layout.trailLength) return;
	// Clear existing curves
	Object.keys(curves).forEach((curve) => {
		if (curves._main_curve.symbol === curves[curve].symbol)
			this.removeCurve(curve);
	});
	const symbol = curves._main_curve.symbol;
	if (length === undefined) length = stx.layout.trailLength;

	const promises = getTrailIntervals(this, length).map((interval) => {
		return new Promise((resolve) => {
			this.addCurve(
				symbol,
				{
					multiplier: interval,
					timeUnit: stx.getPeriodicity().timeUnit
				},
				{},
				resolve
			);
	...
	stx.layout.trailLength = length;

	function getTrailIntervals(crossSection, count = 12) {
		const {
			chart: { defaultPlotField, dataSet }
		} = stx;
		const startDate = crossSection.curves._main_curve.Date;

		let days = [];
		let tickIndex = Math.min(stx.tickFromDate(startDate), dataSet.length - 1);
		let tick;
		do {
			do {
				tick = dataSet[tickIndex];
				tickIndex--;
			} while (tick && !tick[defaultPlotField]);

			days.push(tick.DT);
		} while (tick && days.length < count + 1);

		const [first, ...others] = days;

		return others.map((d) => Math.round((d - first) / CIQ.DAY));
	}
};
...
CIQ.CrossSection.prototype.lockCurve = function (curveId) {
	...
	const curveBOD = new Date(curve.Date);
	curveBOD.setHours(0, 0, 0);
	const mainCurveBOD = new Date(mainCurve.Date);
	mainCurveBOD.setHours(0, 0, 0);
	const msDiff = curveBOD - mainCurveBOD;
	...
	if (xaxisField) {
		return pivotRecordFields(
			...
			(acc, val, i, arr) => {
				...
				if (val[yaxisField] && typeof val[yaxisField] == "object") {
...
/**
 * Gets pixel value from instrument value.
 *
 * @param {number|string} val Instrument value.
 * @param {boolean} [closest] Name of the curve for which the data is pivoted.
 * @return {number|undefined} Pixel value of instrument. Undefined if closest is `false`
 * 		and instrument value is not an exact match.
 *
 * @memberof CIQ.CrossSection
 * @private
 * @since 8.7.0
 */
CIQ.CrossSection.prototype.pixelFromInstrument = function (val) {
	const { stx } = this;

	const { pos } =
		Object.values(stx.chart.dataSegment).find(
			({ instrument }) => instrument === val || instrument === val + "\u200c"
		) || {};

	if (pos === undefined) return;

	const res = stx.chart.left + stx.micropixels + pos;
 ...
 * @since 8.7.0
 */
CIQ.CrossSection.prototype.getCenter = function () {
	const {
		stx,
		center: { x, y }
	} = this;

	const pixelY = y != undefined ? stx.pixelFromTransformedValue(y) : undefined;

	return [this.pixelFromInstrument(x), pixelY];
};

/**
 * Sets crossection chart render type
 *
 * @param {string} type `crosssection` defaults to line rendering, other options are `mountain` and `histogram`  

 * @param {object} [params] Additonal parameters for rendering
 * @param {object} [params.baseline] Baseline parameter applies to histogram rendering and will merge baseline parameters into the yAxis, unless set to null to remove baseline from the yAxis
 * @param {number} [params.baseline.value] Baseline value
 * @param {string} [params.baseline.color] Baseline color. Color needs to be defined for baseline to be drawn.
 * @param {string} [params.baseline.lineWidth] Baseline line width
 * @param {string} [params.baseline.pattern] Baseline pattern
 * @param {number} [params.widthFactor] Histogaram bar width factor, expressed as a decimal value 0 to 1.
 * 
 * @since 9.3.0
 */
CIQ.CrossSection.prototype.setChartType = function (type, params = {}) {
	const validTypes = ["line", "crosssection", "mountain", "histogram"];
	if (!validTypes.includes(type.toLocaleLowerCase())) {
		console.warn("Valid chart rendering types are " + validTypes.join(", "));
		return;
	}
	let ctype = type.replace("line", "crosssection");
	if (ctype !== "crosssection") ctype = "crosssection_" + ctype;

	const { stx } = this;
	if (type === "histogram") {
		if (stx.layout.spacingType === "scaled") stx.layout.spacingType = "uniform";
		if (params.baseline)
			stx.chart.yAxis.baseline = Object.assign(
				stx.chart.yAxis.baseline || {},
				params.baseline
			);
		if (params.baseline === null) delete stx.chart.yAxis.baseline;
		if (params.widthFactor)
			this.initialSettings.widthFactor = params.widthFactor;

		stx.crossSection.setSpacingType("uniform");
	}
	stx.setChartType(ctype);
};
