...
const cssReady = new Promise((resolve) => {
	if (import.meta.webpack) {
		// webpack 5
		import(/* webpackMode: "eager" */ "./visualearnings.css").then(resolve);
	} else if (typeof define === "function" && define.amd) {
		define(["./visualearnings.css"], resolve);
	} else if (typeof window !== "undefined") {
		// no webpack
		CIQ.loadStylesheet(
			new URL("./visualearnings.css", import.meta.url).href,
			resolve
		);
	} else resolve();
}).then((m) => CIQ.addInternalStylesheet(m, "visualearnings")); // a framework, such as Angular, may require style addition
...
Estimize.prototype.drawPriceHorizon = function (stx, quote, x, configuration) {
	...
	if (
		...
		if (impliedPriceW) {
			...
			context.font = "9px Arial, Helvetica, sans-serif";
...
Estimize.prototype.horizonLabel = function (x, y, width, txt, label, stx) {
	...
	context.font = "10px Arial, Helvetica, sans-serif"; //price
	context.fillStyle = "#000000";
	context.fillText(txt, x + 4, y + 3 + fontHeight);

	context.font = "9px Arial, Helvetica, sans-serif"; //label
...
Estimize.prototype.drawDataPanel = function (stx, chart) {
	...
	panel.title.style.display = "none";
	panel.holder.classList.add("estimize_data_panel");
...
Estimize.prototype.go = function (action) {
	...
	if (action == "earnings") {
		if (!ciq.panelExists("estimize_eps")) {
			...
			newPanel.title.style.display = "none";
		...
		if (!ciq.panelExists("estimize_data")) {
			...
			newPanel.title.style.display = "none";
			newPanel.holder.classList.add("estimize_data_panel");
			newPanel.noDrag = true;
 ...
 * @param {HTMLElement} params.container The DOM element to which the Visual Earnings
...
CIQ.VisualEarnings = function (params) {
	this.estimize = params.stx.estimize = new Estimize();
	this.estimize.main(params.stx);
	if (typeof params.container === "string") {
		params.container = params.context.topNode.querySelector(params.container);
	}
	CIQ.UI.VisualEarnings(params.context);

	function clearStyles() {
		params.stx.clearStyles();
	}

	cssReady.then(clearStyles);

	if (params.container) {
		var contextConfig = params.context.config;
		var markup =
			params.markup ||
			(params.deprecatedMenu && config.markupOld) ||
			config.markup;
		var insertContainer =
			params.insertContainer || config.insertContainer || "beforeend";
		var div = document.createElement("div");
		CIQ.innerHTML(div, markup);
		if (params.deprecatedMenu) {
			contextConfig[
				div.querySelector("cq-menu-container").getAttribute("cq-name")
			] = params.menu || config.menuOld;
		} else {
			contextConfig.menus[div.querySelector("cq-menu").getAttribute("config")] =
				params.menu || config.menu;
		}
		params.container.insertAdjacentHTML(insertContainer, markup);
		this.estimize.enable();
	}
};
...
CIQ.UI.VisualEarnings = function (context) {
	const self =
		this instanceof CIQ.UI.VisualEarnings
			? this
			: Object.create(CIQ.UI.VisualEarnings.prototype);

	self.context = context;
	context.advertiseAs(self, "VisualEarnings");

	return self;
};

CIQ.UI.VisualEarnings.prototype.toggle = function (node, type) {
	this.context.stx.estimize.go(type);
};
