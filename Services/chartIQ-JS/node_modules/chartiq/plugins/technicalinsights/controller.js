...
function requestJSON(url, query, cb) {
	CIQ.postAjax({
		...
		cb: function (status, response) {
			if (status !== 200) {
				if (!response)
					return cb({ err: "Error: No response sent from Trading Central." });
				var error = JSON.parse(response);
				return cb({ err: error.message });
			}
 ...
 * @param {CIQ.ChartEngine} stx	Updated chart engine
 */
CIQ.TechnicalInsights.prototype.changeChartEngine = function (stx) {
	this.stx = stx;
	this.panel = stx.chart.panel;
};
...
CIQ.TechnicalInsights.prototype.drawSupportAndResistance = function (
	...
	const supportResistanceDrawInjection = stx.append("draw", function () {
		...
		const srStart = stx.pixelFromDate(srStartDate.DT, chart);
		const srEnd = stx.pixelFromDate(srEndDate.DT, chart);
		let params = {};
 ...
 */
CIQ.TechnicalInsights.prototype.addSupportAndResistanceMouseInjections =
	function ({ interval, term, supportprice, resistanceprice }) {
		const controller = this;
		const { stx } = this;
...
CIQ.TechnicalInsights.prototype.getEvents = function (params, cb) {
	...
	if (parseInt(params.interval, 10)) {
...
CIQ.TechnicalInsights.prototype.getActiveEvents = function () {
	...
	if (["second", "millisecond", "tick"].includes(stx.layout.timeUnit)) {
		return console.error("No Technical Insights for this time interval.");
	}
	...
	function getEvents(symbol, exchange) {
		...
		if (controller.showsr) {
			...
			controller.getSupportAndResistance(params, function (response) {
				if (
					!response.supportAndResistance ||
					!response.supportAndResistance.length
				)
					return;
...
/**
 * Get a y-axisvalue for the given price.
 *
 * @param {Number} price the value to reference
 * @return a y-axisnumber value or null if the price would not be displayed on the panel
 * @private
 */
