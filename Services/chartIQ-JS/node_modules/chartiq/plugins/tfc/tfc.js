 ...
 * @param {string} order.action "buy","sell", “short", “cover"
...
CIQ.Account.prototype.getPoller = function () {
	return {
		...
		update: function (tfc, updateType) {
			this.tfcs = this.tfcs.filter(
				(tfc) => !tfc.stx.destroyed && tfc.container.closest("body")
			);
...
CIQ.TFC = function (config) {
	...
	if (stx.translateUI) stx.translateUI(stx.container.ownerDocument.body);
	...
	if (CIQ.UI) {
		CIQ.UI.KeystrokeHub.addHotKeyHandler(
			...
			({ stx }) => {
				const toggle =
					stx.container.ownerDocument.body.keystrokeHub.context.topNode.querySelector(
						'cq-toggle[member="tfc"], cq-toggle[cq-member="tfc"]'
					);
	...
	this._isOpen = false;
	Object.defineProperty(this, "panelIsOpen", {
		get() {
			return this._isOpen;
		},
		set(value) {
			...
			this._isOpen = value;
			if (!value) this.closeTFC();
			if (this.stx) this.stx.draw();
		}
	});
 ...
 * @memberOf CIQ.TFC
 * @since 7.4.0 Allows the `id` parameter to be a DOM element.
 */
CIQ.TFC.displayDialog = function (id) {
	...
	for (var i = 0; i < registeredContainers.length; i++) {
		var stx = registeredContainers[i].stx;
		if (stx.container.ownerDocument === node.ownerDocument) stx.modalBegin();
	}
...
CIQ.TFC.dismissDialog = function () {
	...
	if (!this.stack.length) {
		...
		for (var i = 0; i < registeredContainers.length; i++) {
			var stx = registeredContainers[i].stx;
			if (stx.container.ownerDocument === node.ownerDocument) stx.modalEnd();
		}
	}
	CIQ.fixScreen();
};
...
CIQ.TFC.prototype.isMobileMode = function () {
	return !!this.chart.container.ownerDocument.querySelector(".break-sm");
};
...
CIQ.TFC.prototype.dragOrdersHeader = function (e) {
	var header = this.panelElements.openOrdersHeader;
...
/**
 * Updates all of the numerical values on the screen including shares, currency, risk/reward, and profit & loss. This method is called
 ...
 * @param  {boolean} isActive True to open panel, false to close.
 */
CIQ.TFC.prototype.openPanel = function (isActive) {
	...
	this.panelIsOpen = isActive;
	this.switchContext(this.context);
	this.enableAccount(this.config.account);
	this.changeSymbol();
};
...
CIQ.TFC.prototype.clearEphemeral = function (which) {
	this.ephemeralNodes[which].forEach((node) => node.remove());
};
...
CIQ.TFC.prototype.renderTrades = function () {
	if (!this.account || !this.panelIsOpen) return;
	var trades = this.account.trades[this.stx.chart.symbol];
 ...
 * The ref parameter is used to indicate that updateData is used for display synchronization and contains the TFC instance that instantiated synchronization
 */
CIQ.TFC.prototype.updateData = function (ref) {
	...
	function update() {
		...
		self.panelElements.currentCash.innerHTML = CIQ.money(
			self.account.balances.cash,
			0,
			CIQ.convertCurrencyCode(self.account.currency)
		);
		self.panelElements.currentFunds.innerHTML = CIQ.money(
		...
		self.panelElements.currentPosition.innerHTML = CIQ.commas(
			position.quantity
		);

		// Build balances table
		self.panelElements.liquidity.innerHTML = CIQ.money(
			self.account.balances.liquidity,
			null,
			CIQ.convertCurrencyCode(self.account.currency)
		);
		self.panelElements.unsettledCash.innerHTML = CIQ.money(
			self.account.balances.unsettledCash,
			null,
			CIQ.convertCurrencyCode(self.account.currency)
		);
		self.panelElements.cashSweep.innerHTML = CIQ.money(
			self.account.balances.cash,
			null,
			CIQ.convertCurrencyCode(self.account.currency)
		);
		if (self.account.balances.profitLoss) {
			self.panelElements.profitLoss.parentNode.style.display = "";
			self.panelElements.profitLoss.innerHTML = CIQ.money(
				self.account.balances.profitLoss,
				null,
				CIQ.convertCurrencyCode(self.account.currency)
			);
		} else {
			self.panelElements.profitLoss.parentNode.style.display = "none";
		}
		self.panelElements.buyingPower.innerHTML = CIQ.money(
			self.account.balances.buyingPower,
			null,
			CIQ.convertCurrencyCode(self.account.currency)
		);
		Object.values(self.elements)
			.concat(Object.values(self.panelElements))
			.forEach(function (elem) {
				if (elem && !elem.length && !elem.children.length)
					// ignore NodeLists for now
					CIQ.makeTranslatableElement(elem, self.stx, elem.innerHTML);
			});
		...
		if (!foundOne) {
			CIQ.makeTranslatableElement(
				CIQ.newChild(CIQ.newChild(table, "TR"), "TD", "tfc-not-found"),
				self.stx,
				"No Open Orders"
			);
		}
		...
		for (j = 0; j < symbolList.length; j++) {
			...
			if (viewType == "lots") {
				...
				CIQ.makeTranslatableElement(td, self.stx, "Close Position");
				if (position.quantity > 0) {
				...
				for (var t = 0; t < self.account.trades[symbol].length; t++) {
					...
					if (viewType == "lots" || viewType == "maintenance") {
						...
						if (viewType == "maintenance") {
							...
							td = CIQ.newChild(tr, "TD", "stx-btn click");
							CIQ.makeTranslatableElement(td, self.stx, "Close");
							if (trade.quantity > 0) {
		...
		if (!foundOne) {
			...
			CIQ.makeTranslatableElement(td, self.stx, "No Positions");
			//td.setAttribute("colspan", 5);
			self.panelElements.closeAllPositions.style.display = "none";
			...
			if (self.account.config.closeAll)
				self.panelElements.closeAllPositions.style.display = "";
			self.container.querySelector(
				".stx-trade-positions .tfc-positions-view.holder"
			).style.display = "";
			self.container.querySelector(".stx-trade-positions thead").style.display =
		...
		if (self.account.config.tradeActions) {
			self.panelElements.positionsViewMaintenance.style.display = "";
		} else {
			self.panelElements.positionsViewMaintenance.style.display = "none";
		}
...
CIQ.TFC.prototype.refreshScrollWindows = function (openOrderTop) {
	...
	if (!openOrderTop)
		openOrderTop = this.panelElements.openOrdersHeader.resizedTop;
	if (openOrderTop) {
	...
	if (!positionCount || positionHeight < 60) {
		positionHeight = 60;
		openOrderHeight = totalHeight - positionHeight;
		this.panelElements.openOrdersHeader.classList.add("no-resize");
		...
		this.panelElements.openOrdersHeader.classList.remove("no-resize");
	}
...
/**
 * The TFC menu is dynamic based on whether the account contains a current position for the security, and whether that position is
...
CIQ.TFC.prototype.cancelOpenOrder = function () {
	...
	var confirmDialog = this.dqs(".tfcConfirmOrder");
	...
	this.account.confirmOrder(this, order, function (confirmation) {
		var confirmDialog = self.dqs(".tfcConfirmOrder");
		...
		tifNode.innerHTML = "";
		if (order.tif == "DAY")
			CIQ.makeTranslatableElement(tifNode, self.stx, "Day Order");
		else if (order.tif == "GTC")
			CIQ.makeTranslatableElement(tifNode, self.stx, "Good Until Cancelled");
...
CIQ.TFC.prototype.confirmReplace = function (order) {
	var confirmReplaceDialog = this.dqs(".tfcConfirmReplace");
	...
	function closure(self, order, submitButton) {
		return function () {
			CIQ.clearSafeClickTouches(submitButton);
			CIQ.clearSafeClickTouches(abandonButton);
			var confirmReplaceDialog = self.dqs(".tfcConfirmReplace");
...
CIQ.TFC.prototype.confirmOrder = function (order) {
	var self = this;
	this.modalEnd();
	var confirmOrder = this.dqs(".tfcConfirmOrder");
	...
	this.account.confirmOrder(this, order, function (confirmation) {
		var confirmOrder = self.dqs(".tfcConfirmOrder");
...
CIQ.TFC.prototype.confirmCloseAllPositions = function () {
	var self = this;
	this.modalEnd();
	var confirmCloseAll = this.dqs(".tfcConfirmCloseAll");
	...
	function closure() {
		CIQ.clearSafeClickTouches(submitButton);
		CIQ.clearSafeClickTouches(abandonButton);
		var confirmCloseAll = self.dqs(".tfcConfirmCloseAll");
		confirmCloseAll.querySelector(".processOrder").style.display = "block";
...
CIQ.TFC.prototype.confirmOCO = function (order, type) {
	...
	var confirmOCO = this.dqs(".tfcConfirmOCO");
	...
	for (i = 0; i < order.length; i++) {
		...
		if (type == "bracket") {
			if (oco.stop)
				CIQ.makeTranslatableElement(orderLabels[i], this.stx, "Stop Loss");
			else if (oco.limit)
				CIQ.makeTranslatableElement(orderLabels[i], this.stx, "Take Profit");
			orderLabels[i].appendChild(document.createTextNode(":"));
			CIQ.makeTranslatableElement(
				headerNode,
				this.stx,
				"Confirm: Bracket / Protect"
			);
		} else {
			CIQ.makeTranslatableElement(orderLabels[i], this.stx, "Order");
			orderLabels[i].appendChild(document.createTextNode(" " + (i + 1) + ":"));
			CIQ.makeTranslatableElement(
				headerNode,
				this.stx,
				"Confirm: One Cancels the Other"
			);
		}
	}
	...
	tifNode.innerHTML = "";
	if (order[0].tif == "DAY")
		CIQ.makeTranslatableElement(tifNode, this.stx, "Day Order");
	else if (order[0].tif == "GTC")
		CIQ.makeTranslatableElement(tifNode, this.stx, "Good Until Cancelled");
	...
	function closure(self, order, submitButton) {
		return function () {
			CIQ.clearSafeClickTouches(submitButton);
			CIQ.clearSafeClickTouches(abandonButton);
			var confirmOCO = self.dqs(".tfcConfirmCloseAll");
 ...
 */
CIQ.TFC.prototype.updateAccountUI = function () {
	if (!this.account) return;
	const { oto, gtcOnly, tradeActions } = this.account.config;
	const els = this.elements;

	els.addOTOStop.style.display = oto ? "" : "none";

	els.limitDay.disabled = !!gtcOnly;
	els.limitDay.selected = !gtcOnly;
	els.ocoDay.disabled = !!gtcOnly;
	els.ocoDay.selected = !gtcOnly;
	this.panelElements.positionsViewMaintenance.style.display = tradeActions
		? ""
		: "none";
};
/**
...
/**
 * Adds drag handler to target
 *
 * @param {string} type drag handler type
 * @param {HTMLElement} target drag target
 * @param {HTMLElement} dragNode drag node
 * @return {object} Safety object which can be passed to CIQ.safeClickTouch
 * @private
 * @since 8.9.0
 */
CIQ.TFC.prototype._addSafeDrag = function (type, target, dragNode) {
	return CIQ.safeDrag(
		target,
		(e) => this.startDrag(e, dragNode || target),
		(e) => this[type](e),
		(e) => this.endDrag(e, dragNode || target)
	);
};
 ...
 *
 * @param {object[]} items Items consisting of element, action and parameters
 * @private
 * @since 8.9.0
 */
CIQ.TFC.prototype._addSafeClickTouch = function (items) {
	items.forEach(({ el, action, params }) => {
		CIQ.safeClickTouch(
			el,
			() => {
				// const isUserAction = /_/.test(action);
				const isMethod = action in this;
				if (!isMethod) this.userAction(action);
				else this[action]();
			},
			params
		);
...
/**
 * Adds focus to the clicked element in item array and invokes setActiveInput method
 *
 * @param {object[]} items Items consisting of element and type
 * @private
 * @since 8.9.0
 */
CIQ.TFC.prototype._setFocused = function (items) {
	items.forEach(({ el, type }) => {
		CIQ.safeClickTouch(
			el,
			(e) => {
				e.target.focus();
				this.setActiveInput(type);
			},
...
/**
 * Adds focus to the clicked element and prevents default event hadler if CIQ.isIOS7or8 is truthy
 *
 * @param {HTMLElement} el Element to apply handler
 * @private
 * @since 8.9.0
 */
CIQ.TFC.prototype._setFocusedIsIOS7or8 = function (el) {
	CIQ.safeClickTouch(el, (e) => {
		if (CIQ.isIOS7or8) {
			e.preventDefault();
			e.target.focus();
		}
...
/**
 * Adds change event listeners to listed items
 *
 * @param {object[]} items Items consisting of element and method to apply on change
 * @private
 * @since 8.9.0
 */
CIQ.TFC.prototype._addChangeListeners = function (items) {
	items.forEach(({ el, method = "updateValues" }) => {
		el.addEventListener("change", () => this[method]());
	});
};

/**
 * Abandons started operation such as market order when clicked on element in item list
 *
 * @param {HTMLElement[]} items HTMLElement array
 * @private
 * @since 8.9.0
 */
CIQ.TFC.prototype._addAbandon = function (items) {
	items.forEach((el) => {
		CIQ.safeClickTouch(el, () => this.closeTFC());
	});
};
...
CIQ.TFC.prototype.changePositionsView = function (viewType) {
	this.container
		.querySelectorAll(".tfc-positions-view")
		.forEach(({ classList }) =>
			classList.toggle("active", classList.contains(viewType))
		);
 ...
 * @param {object} params Configuratioon parameters
 ...
 * @since 8.9.0
 */
CIQ.TFC.prototype.setupChartControls = function ({ context, stx }) {
	...
	const dom = this.dom;
	dom.dragLineAbove = cqs(".drag-price-line").cloneNode(true);
	dom.dragLineCenter = cqs(".drag-price-line").cloneNode(true);
	dom.dragLineBelow = cqs(".drag-price-line").cloneNode(true);
	dom.marketOrder = cqs(".market-order").cloneNode(true);
	dom.limitOrder = cqs(".stx-limit-order").cloneNode(true);
	dom.otoAbove = cqs(".OTO.stx-stop").cloneNode(true);
	dom.otoBelow = cqs(".OTO.stx-stop").cloneNode(true);
	dom.ocoOrder = cqs(".stx-oco-order").cloneNode(true);
	dom.ocoAbove = cqs(".oco.tfc-oco-above").cloneNode(true);
	dom.ocoBelow = cqs(".oco.tfc-oco-below").cloneNode(true);
	dom.shadeAbove = cqs(".tfc-shade").cloneNode(true);
	dom.shadeBelow = cqs(".tfc-shade").cloneNode(true);
	...
	componentHolder.classList.add("ciq-tfc-components");
	this.holder.parentNode.append(componentHolder);
	for (const componentName in dom) {
		const component = this.dom[componentName];
	...
	const els = this.elements;
	const marketOrder = this.dom.marketOrder;
	let qs = marketOrder.querySelector.bind(marketOrder);
	els.marketBuy = qs(".tfc-market-buy-action");
	els.marketSell = qs(".tfc-market-sell-action");
	els.marketCurrency = qs("input.tfc-currency");
	els.marketShares = qs("input.tfc-shares");
	els.marketBracket = qs(".tfc-market-section.complex");
	els.marketLossBracketDifferential = qs(
		".tfc-market-section.complex input.tfc-market-loss-bracket"
	);
	els.marketProfitBracketDifferential = qs(
		".tfc-market-section.complex input.tfc-market-profit-bracket"
	);
	els.askForexPart = qs(".tfc-ask strong");
	els.bidForexPart = qs(".tfc-bid strong");
	els.askEquityPart = qs(".tfc-ask span");
	els.bidEquityPart = qs(".tfc-bid span");
	els.abandonMarketOrder = qs(".stx-btn.stx-ico");

	const limitOrder = this.dom.limitOrder;
	qs = limitOrder.querySelector.bind(limitOrder);
	els.limitBuy = qs(".click.tfc-buy");
	els.limitReplace = qs(".click.tfc-replace");
	els.limitSell = qs(".click.tfc-sell");
	els.limitShort = qs(".click.tfc-short");
	els.limitCover = qs(".click.tfc-cover");
	els.addOTOStop = qs(".OTO.stop");
	els.addOTOLimit = qs(".OTO.limit");
	els.sharesOwned = qs(".tfc-shares-owned span");
	els.limitShares = qs("input.tfc-shares");
	els.limitCurrency = qs("input.tfc-currency");
	els.gainAmount = qs(".tfc-gain-amount");
	els.gainPercent = qs(".tfc-gain-percent");
	els.limitRiskReward = qs(".risk-reward .stx-data");
	els.limitTIF = qs("select");
	els.limitDay = qs("select .tfc-day");
	els.abandonLimitOrder = qs(".stx-btn.stx-ico");
	els.cancelLimitOrder = qs(".tfc-cancel-button");
	els.cancelDescription = qs(".tfc-cancel-description");

	els.dragLineAbovePrice = dom.dragLineAbove.querySelector(".tfc-price");
	els.dragLineCenterPrice = dom.dragLineCenter.querySelector(".tfc-price");
	els.dragLineBelowPrice = dom.dragLineBelow.querySelector(".tfc-price");

	const otoAbove = dom.otoAbove;
	qs = otoAbove.querySelector.bind(otoAbove);
	els.removeOTOAbove = qs(".stx-btn.stx-ico .stx-ico-close");
	els.aboveGainAmount = qs(".tfc-gain-amount");
	els.aboveGainPercent = qs(".tfc-gain-percent");
	els.otoAboveLegLabel = qs(".tfc-oto-leg-label");

	qs = dom.otoBelow.querySelector.bind(dom.otoBelow);
	els.removeOTOBelow = qs(".stx-btn.stx-ico .stx-ico-close");
	els.belowGainAmount = qs(".tfc-gain-amount");
	els.belowGainPercent = qs(".tfc-gain-percent");
	els.otoBelowLegLabel = qs(".tfc-oto-leg-label");

	qs = dom.ocoOrder.querySelector.bind(dom.ocoOrder);
	this.elements.ocoRiskReward = qs(".inputTemplate .risk-reward");
	els.ocoGainAmount = qs(".tfc-gain-amount");
	els.ocoGainPercent = qs(".tfc-gain-percent");
	els.ocoCurrency = qs(".stx-data input");
	els.ocoTrade = qs(".click.oco");
	els.ocoTIF = qs("select");
	els.ocoDay = qs("select .tfc-day");
	els.bracketId = qs(".tfc-bracketid");
	els.abandonOCOOrder = qs(".stx-btn.stx-ico");

	qs = dom.ocoAbove.querySelector.bind(dom.ocoAbove);
	els.ocoAboveHead = qs(".stx-head");
	els.ocoAboveShares = qs(".stx-data span");

	qs = dom.ocoBelow.querySelector.bind(dom.ocoBelow);
	els.ocoBelowHead = qs(".stx-head");
	els.ocoBelowShares = qs(".stx-data span");

	const cqsa = container.querySelectorAll.bind(container);
	els.useDollarsLabel = cqsa(".stx-label.tfc-price-dollars");
	els.useAmountLabel = cqsa(".stx-label.tfc-price-amt");
	els.useSharesLabel = cqsa(".stx-label.tfc-shares-to-buy");
	els.useUnitsLabel = cqsa(".stx-label.tfc-qty-to-buy");
	els.usePipsLabel = cqsa(".stx-label.tfc-pips");
	els.usePointsLabel = cqsa(".stx-label.tfc-points");

	let safety = this._addSafeDrag("dragMarketOrder", dom.marketOrder);
	let sharedParams = { safety, absorbDownEvent: false };
	this._addSafeClickTouch([
		{ el: els.marketBuy, action: "market_buy", params: sharedParams },
		{ el: els.marketSell, action: "market_sell", params: sharedParams },
		{ el: els.limitBuy, action: "limit_buy", params: sharedParams },
		{ el: els.limitSell, action: "limit_sell", params: sharedParams },
		{ el: els.limitShort, action: "limit_short", params: sharedParams },
		{ el: els.limitCover, action: "limit_cover", params: sharedParams },
		{ el: els.limitReplace, action: "replace", params: sharedParams },
		{ el: els.ocoTrade, action: "OCO", params: sharedParams }
	]);

	this._addSafeDrag("dragCenterLine", dom.dragLineCenter);
	safety = this._addSafeDrag(
		"dragCenterLine",
		dom.limitOrder,
		dom.dragLineCenter
	);
	sharedParams = { safety, absorbDownEvent: false };

	this._addSafeDrag("dragAboveLine", dom.dragLineAbove);
	this._addSafeDrag("dragAboveLine", dom.ocoAbove, dom.dragLineAbove);
	this._addSafeDrag("dragAboveLine", dom.otoAbove, dom.dragLineAbove);

	this._addSafeDrag("dragBelowLine", dom.dragLineBelow);
	...
	this._addSafeDrag("dragBelowLine", dom.ocoBelow, dom.dragLineBelow);
	this._addSafeDrag("dragBelowLine", dom.otoBelow, dom.dragLineBelow);

	this._addSafeClickTouch([
		{ el: els.addOTOStop, action: "addOTOStop", params: sharedParams },
		{ el: els.removeOTOAbove, action: "removeOTOAbove" },
		{ el: els.addOTOLimit, action: "addOTOLimit", params: sharedParams },
		{ el: els.removeOTOBelow, action: "removeOTOBelow" },
		{ el: els.cancelLimitOrder, action: "cancelOpenOrder" }
	]);

	this._setFocused([
		{ el: els.limitShares, type: "shares" },
		{ el: els.limitCurrency, type: "currency" },
		{ el: els.marketShares, type: "shares" },
		{ el: els.marketCurrency, type: "currency" }
	]);

	this._setFocusedIsIOS7or8(els.marketLossBracketDifferential);
	this._setFocusedIsIOS7or8(els.marketProfitBracketDifferential);

	this._addChangeListeners([
		{ el: els.limitShares },
		{ el: els.limitCurrency },
		{ el: els.limitTIF, method: "tifHasChanged" },
		{ el: els.marketShares },
		{ el: els.marketCurrency },
		{ el: els.marketLossBracketDifferential },
		{ el: els.marketProfitBracketDifferential },
		{ el: els.ocoCurrency }
	]);

	this._addAbandon([
		els.abandonLimitOrder,
		els.abandonMarketOrder,
		els.abandonOCOOrder
	]);
...
CIQ.TFC.prototype.construct = function (config) {
	this.config = config;
	const { stx, context } = config;

	const { multiChartContainer } = context.topNode;

	if (multiChartContainer) multiChartContainer.plugins.push(this);

	var topNode = context
		? multiChartContainer || context.topNode
		: stx.container.ownerDocument;
	const tfcContainer = (this.tfcContainer =
		topNode.querySelector(".tfc.container"));
	let container =
		(CIQ.UI && CIQ.UI.closestContextContainer(tfcContainer)) ||
		tfcContainer.closest("cq-context");
	if (container.multiChartContainer) container = container.multiChartContainer;

	this.container = container;
	this.width = tfcContainer.offsetWidth;

	this.switchContext(
		(multiChartContainer && multiChartContainer.activeContext) || context,
		config.chart
	);

	const channelEngine = multiChartContainer
		? multiChartContainer.CIQ.UI.context.stx
		: stx;

	this.selectSymbol = function (symbol) {
		if (this.context)
			this.context.changeSymbol({ symbol: symbol.toUpperCase() });
	};

	// start of tfc panel elements
	const panelEls = this.panelElements;

	const cqs = container.querySelector.bind(container);
	const cqsa = container.querySelectorAll.bind(container);
	const tradePanel = cqs(".stx-trade-panel");
	const tpqs = tradePanel.querySelector.bind(tradePanel);

	panelEls.dialogContainer = cqs(".stx-dialog-container.trade");
	this.dqs = panelEls.dialogContainer.querySelector.bind(
		panelEls.dialogContainer
	);

	if (multiChartContainer) {
		multiChartContainer
			.querySelector(".ciq-multi-chart-container-wrapper")
			.append(panelEls.dialogContainer);
	}

	panelEls.closeAllPositions = tpqs(".stx-btn.stx-close-all");
	panelEls.positionsViewSummary = tpqs(".tfc-positions-view.summary");
	panelEls.positionsViewLots = tpqs(".tfc-positions-view.lots");
	panelEls.positionsViewPerformance = tpqs(".tfc-positions-view.performance");
	panelEls.positionsViewMaintenance = tpqs(".tfc-positions-view.maintenance");

	this.templates.openOrderMarker = cqs(".tfc .open-order-marker");

	var menu = this.menu;
	menu.enableMarket.nodes = cqsa(".stx-trade-panel .stx-market");
	menu.enableBuy.nodes = cqsa(".stx-trade-panel .stx-buy");
	menu.enableSell.nodes = cqsa(".stx-trade-panel .stx-sell");
	menu.enableShort.nodes = cqsa(".stx-trade-panel .stx-short");
	menu.enableCover.nodes = cqsa(".stx-trade-panel .stx-cover");
	menu.enableStraddle.nodes = cqsa(".stx-trade-panel .stx-straddle");
	menu.enableStrangle.nodes = cqsa(".stx-trade-panel .stx-strangle");
	menu.enableBracket.nodes = cqsa(".stx-trade-panel .stx-bracket");

	this.establishMenu();

	panelEls.currentCash = cqs(".stx-trade-panel .tfc-current-cash");
	panelEls.currentFunds = cqs(".stx-trade-panel .tfc-current-funds");
	panelEls.currentPosition = cqs(".stx-trade-panel .tfc-current-position");
	panelEls.openOrdersHeader = cqs(".stx-trade-current .stx-head-bar");
	panelEls.liquidity = cqs(".stx-trade-account .tfc-liquidity");
	panelEls.unsettledCash = cqs(".stx-trade-account .tfc-unsettled-cash");
	panelEls.cashSweep = cqs(".stx-trade-account .tfc-cash");
	panelEls.profitLoss = cqs(".stx-trade-account .tfc-profitloss");
	panelEls.buyingPower = cqs(".stx-trade-account .tfc-buying-power");

	function addChangePosition(self, items) {
		items.forEach(({ el, view }) => {
			CIQ.safeClickTouch(el, () => self.changePositionsView(view));
		});
	}

	this._addSafeClickTouch([
		{ el: panelEls.closeAllPositions, action: "confirmCloseAllPositions" }
	]);

	addChangePosition(this, [
		{ el: panelEls.positionsViewSummary, view: "summary" },
		{ el: panelEls.positionsViewLots, view: "lots" },
		{ el: panelEls.positionsViewPerformance, view: "performance" },
		{ el: panelEls.positionsViewMaintenance, view: "maintenance" }
	]);

	CIQ.safeDrag(
		panelEls.openOrdersHeader,
		(e) => this.startDragOrdersHeader(e, panelEls.openOrdersHeader),
		(e) => this.dragOrdersHeader(e),
		(e) => this.endDrag(e, panelEls.openOrdersHeader)
	);

	const resizeListener = () => this.refreshScrollWindows();

	this.context.topNode.ownerDocument.defaultView.addEventListener(
		"resize",
		resizeListener,
		false
	);

	const masterChart = multiChartContainer
		? multiChartContainer.querySelector(".chartContainer")
		: this.context.stx;

	masterChart.addEventListener("destroy", function (obj) {
		obj.stx.container.ownerDocument.defaultView.removeEventListener(
			"resize",
			resizeListener
		);
	});
	...
	CIQ.safeClickTouch(
		...
		switchToInfoView(true, this)
	);
	CIQ.safeClickTouch(
		container.querySelector(".stx-trade-info .stx-trade-ticket-toggle"),
		switchToInfoView(false, this)
	);

	if (CIQ.UI)
		this.unsubscribeTfcChannnel =
			CIQ.UI.BaseComponent.prototype.channelSubscribe(
				(context.config.channels || {}).tfc || "channel.tfc",
				(isActive) => {
					if (multiChartContainer) {
						const soloOnTFC = CIQ.getFnFromNS(
							context,
							"config.soloActive.onTFC.shouldSolo"
						);
						const activeChartEngine = multiChartContainer.activeContext.stx;
						if (isActive && !activeChartEngine.chart.symbol) return;
						if (soloOnTFC(activeChartEngine) || !isActive)
							multiChartContainer.soloActive(isActive, "onTFC");
					}
					this.openPanel(isActive);
				},
				channelEngine
			);

	function switchToInfoView(large, self) {
		return function () {
			container
				.querySelector(".stx-trade-nav")
 ...
 * @since 8.9.0
 */
CIQ.TFC.prototype.switchContext = function (context) {
	if (this.account) this.closeTFC();

	this.context = context;
	if (this.previousContext) {
		// cleanup previous context
		this.context.stx.removeInjection(this.onDraw);
		this.context.stx.removeEventListener(this.onNewSymbol);
	}
	...
	this.chart = context.stx.chart;
	this.holder = this.stx.chart.panel.holder;

	let contextContainer = this.contexts.get(context);
	if (contextContainer) {
		this.dom = contextContainer.dom;
		this.elements = contextContainer.elements;
	} else {
		this.dom = {};
		this.elements = {};
		this.setupChartControls({ context, stx });
		contextContainer = {
			dom: this.dom,
			elements: this.elements
		};
		this.contexts.set(context, contextContainer);
	}
	if (this.account) this.updateData();

	this.onDraw = this.stx.append("draw", () => {
		if (this.panelIsOpen) this.render();
	});

	this.onNewSymbol = stx.addEventListener("newChart", () => {
		this.changeSymbol();
	});
};
